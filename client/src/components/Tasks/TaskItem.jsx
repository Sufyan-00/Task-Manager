    ...
    >
      <div className="task-item-header">
        <div className="task-item-main">
          <div className="task-title-section">
            ...
            <div className="task-title-wrapper">
              ...
              <div className="task-indicators">
                {taskIsShared && (
                  <span className="task-indicator shared-indicator" title={`Shared by ${getTaskOwnerEmail(task)}`}>
                ...
                <span className={`task-indicator priority-indicator priority-${task.priority}`} title={`Priority: ${task.priority}`}>
          ...
          <div className="progress-section">
            <div className="progress-label">
              <span className="progress-percent">{task.progress || 0}%</span> Complete
            </div>
            <div className="progress-bar">
              <div 
                className={`progress-fill ${(task.progress || 0) === 100 ? 'completed' : ''}`}
          ...
          <div className="task-meta">
            {task.dueDate && (
              <span className={`task-meta-item due-date ${dueDatePassed && !task.completed ? 'overdue' : ''}`}>
            ...
            {task.notify && (
              <span className="task-meta-item notifications" title="Notification enabled">
                <span className="meta-icon">Notify</span>
              </span>
            )}
          ...
          {task.subtasks && task.subtasks.length > 0 && (
            <div className="subtasks-section">
              <div className="subtasks-title">
                Subtasks <span className="subtask-count">({task.subtasks.filter(st => st.completed).length}/{task.subtasks.length})</span>
              </div>
                  ...
                  <div key={subtask._id} className="subtask-item">
                    <div className="subtask-checkbox-wrapper">
                      <input
                        ...
                        onChange={(e) => handleSubtaskToggle(task._id, subtask._id, e.target.checked)}
                        disabled={!userCanEdit}
                        className="subtask-checkbox"
                      />
                      <label htmlFor={`subtask-${subtask._id}`} className="subtask-checkbox-custom"></label>
                    </div>
          ...
          {userIsOwner && task.sharedWith && task.sharedWith.length > 0 && (
            <div className="sharing-section">
              <div className="sharing-title">
                Shared with <span className="sharing-count">({task.sharedWith.length})</span>
              </div>
              <div className="shared-users-list">
                {task.sharedWith.map((share) => (
                  <div key={share.user?._id || share._id} className="shared-user">
                    <span className="shared-user-info">
                      <span className="user-email">{share.user?.email || 'unknown'}</span>
                      <span className="user-permission">{share.permission}</span>
                    </span>
                    <button 
                      onClick={() => handleUnshare(task._id, share.user?._id || share._id)}
                      className="unshare-btn"
                      aria-label={`Remove access for ${share.user?.email || 'unknown'}`}
                    >
        ...
        <div className="task-item-actions">
          ...
          {userIsOwner && (
            <button 
              onClick={() => setShowShareForm(showShareForm === task._id ? null : task._id)}
              className={`task-action-btn share-btn ${showShareForm === task._id ? 'active' : ''}`}
              aria-label="Share task"
              title="Share task"
            >
              <span className="action-text">{showShareForm === task._id ? 'Cancel' : 'Share'}</span>
            </button>
          )}
        </div>
      </div>
